...
public class PlayerThirstProvider implements ICapabilityProvider, INBTSerializable<CompoundTag> {
    ...
    private final LazyOptional<PlayerThirst> optional = LazyOptional.of(this::createPlayerThist);

    private PlayerThirst createPlayerThist() {
        if (this.thirst == null){
    ...
    public CompoundTag serializeNBT() {
        ...
        createPlayerThist().saveNBTData(nbt);
        return nbt;
    }

    @Override
    public void deserializeNBT(CompoundTag nbt) {
        createPlayerThist().loadNBTData(nbt);
    }
}
